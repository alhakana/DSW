...
public class PageView extends JPanel implements IListener {
	...
	private DocumentView containerParent;
	private JLabel label;
	private StateManager manager;
	private CommandManager commandManager;
	private List<SlotView> slots;
	private List<SlotView> selectedSlots;
	...
	public PageView(Page pageModel, DocumentView containerParent) {
		...
		selectedSlots = new ArrayList<SlotView>();
		slots = new ArrayList<SlotView>();
	...
	public void update(Notification notification) {
		if (notification.getNotificationCode() != null) {
			...
			case DELETE_ME:
				...
				break;
			case NEW_SLOT:
				addSlot((RuNode)notification.getObject(), false);
	...
	public void addSlot(RuNode slot, boolean selected) {
	...
	public void focus() {
		if (MainFrame.getInstance().getTree().getPathRuTreeItem(1).getNodeModel().equals(containerParent.getContainerParent().getProjectModel()))
			containerParent.putMeInFocus();
	}
	
	private void deleteMe() {
		
		//ovaj uslov obezbedjuje da ce ako se nalazi u podeljenom dokumentu tj imamo vise istih pageview-a
		//samo jednom da se pozove ovo iz linije 228
		//sto ce svakako da dovede do toga da se obrisu svi page-ovi koji su isti u okviru podeljenih dokumenata
		if(pageModel.getParent().getParent().getName().equals(containerParent.getContainerParent().getProjectModel().getName())) 
			MainFrame.getInstance().getTree().getPathRuTreeItem(2).remove(MainFrame.getInstance().getTree().getPathRuTreeItem(2).getIndex(MainFrame.getInstance().getTree().getSelectedRuTreeItem()));
		...
		((ProjectView)containerParent.getContainerParent()).repaint();
